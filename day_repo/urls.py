...
urlpatterns = [
    ...
    # 詳細表示(パラメータにPK)
    path("detail/<slug:slug>/", ReportModelDetailView.as_view(), name="day_repo_detail"),
    # 日報作成
    path("create/", ReportModelFormCreateView.as_view(), name="day_repo_create"),
    # 日報編集
    path("edit/<slug:slug>/", ReportModelFormUpdateView.as_view(), name="day_repo_edit"),
    # 日報削除
    path("delete/<slug:slug>/", ReportModelFormDeleteView.as_view(), name="day_repo_delete"),
    # 画像アップロード画面
